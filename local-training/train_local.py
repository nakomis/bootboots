#!/usr/bin/env python3
"""
BootBoots - Local Training Script (M-series Mac)

Trains a MobileNetV2-based binary classifier (Boots vs NotBoots) on locally
downloaded images. Expects the directory structure created by download_data.py.

Run with:
    python train_local.py

Or with custom settings:
    python train_local.py --data data --epochs 30 --batch-size 32
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train BootBoots cat classifier locally")
    parser.add_argument("--data", default="data", help="Data directory (default: data)")
    parser.add_argument("--output", default="models", help="Output directory for saved model (default: models)")
    ...
    # Log which Metal/CPU device TF is using
    ...
    # Count images per class to compute weights that penalise rare-class errors more
    ...
    history = model.fit(train_ds, validation_data=val_ds, epochs=args.epochs, callbacks=cb_list,
